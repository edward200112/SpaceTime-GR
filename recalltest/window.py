...
# ----------------------------
# Streaming sample from huge JSONL (reservoir sampling)
# ----------------------------
def reservoir_sample_jsonl(path: str, sample_size: int, seed: int = 42, max_read_lines: int = -1) -> List[Dict]:
    ...
    Avoids loading entire 2.2M lines into memory.
    """
...
def merge_adapter_into_base(base_model, adapter_dir: str):
    """
    base -> load adapter -> merge -> return HF model (merged weights)
    """
...
def save_and_reload_pure_model(model, tokenizer, out_dir: str):
    """
    Save merged weights and reload as a clean AutoModelForCausalLM to avoid 'multiple adapters' issues.
    """
...
# ----------------------------
# Recall@K evaluation (generation-based) + rolling window
# ----------------------------
...
def recall_at_k_generate(
    ...
    for i in pbar:
        ...
        # 关键：left padding 下，生成部分从 padded input_len 之后开始
        ...
        for b in range(len(batch)):
            ...
            if len(preds) == 0:
                ...
                this_hit = 1 if (tgt in uniq) else 0
                hit += this_hit
                total += 1
                recent_hits.append(this_hit)

            avg = (hit / total) if total else 0.0
            roll = (sum(recent_hits) / len(recent_hits)) if recent_hits else 0.0
    ...
    return {
        ...
        f"not_enough_unique_top{k}_count": not_enough_unique,
        "beams": beams,
...
def main():
    ...
    # 3) Evaluate SFT (Base -> SFT adapter -> merge -> reload)
    ...
    # 4) Evaluate GRPO (Base -> SFT merge -> reload -> GRPO adapter)
