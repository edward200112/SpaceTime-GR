"""
Evaluate SFT Model Quality (Fixed)
"""
...
class SFTEvaluator:
    def __init__(self, base_model, sft_ckpt, mapping_file, device="cuda"):
        self.device = device
        # [Fix] 保存 checkpoint 路径到 self，以便后续打印
        self.sft_ckpt_path = sft_ckpt 
        
        self.load_mapping(mapping_file)
        self.load_model(base_model, sft_ckpt)
    ...
    def load_mapping(self, mapping_file):
        ...
        print(f"Loaded {len(self.tree_map)} POIs. Valid Cities: {len(self.city_sid_strings)}")

    def load_model(self, base, sft):
    ...
    def evaluate(self, test_file, num_samples=500, beams=10):
        ...
        print(f"Evaluating {len(test_data)} samples with Beams={beams}...")
        ...
        print("\n" + "="*40)
        print(f"SFT EVALUATION RESULTS (N={n})")
        # [Fix] 使用 self.sft_ckpt_path
        print(f"Checkpoint: {self.sft_ckpt_path}") 
        print("="*40)
...
if __name__ == "__main__":
    ...
    SFT_CKPT = "/workspace/data/llm_ckpt_sft_v5_balanced/checkpoint-2000"
    ...
    if not os.path.exists(SFT_CKPT):
        print(f"Checkpoint not found: {SFT_CKPT}")
        # 自动回退逻辑
        SFT_CKPT = "/workspace/data/llm_ckpt_sft_v2_optimized/checkpoint-34800"
        
    evaluator = SFTEvaluator(BASE, SFT_CKPT, MAP)
    evaluator.evaluate(TEST_DATA, num_samples=500, beams=10)