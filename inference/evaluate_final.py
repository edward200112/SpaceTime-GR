"""
Final Evaluation for GRPO v4.1 Breadcrumbs
"""
...
class GRPOEvaluator:
    ...
    def load_model(self, base, sft, grpo):
        ...
        # 注意：这里我们加载 GRPO 的 LoRA，不再 merge，保持灵活性
    ...
    def evaluate(self, test_file, num_samples=500, beams=10):
        print(f"Evaluating {num_samples} samples from {test_file}...")
        data = []
        ...
        print("\n" + "="*40)
        print(f"FINAL GRPO V4.1 RESULTS (N={n})")
        print(f"SFT Base: {self.sft_path}")
        print(f"GRPO Adapter: {self.grpo_path}")
        print("="*40)
        ...
        print(f"Layer 1 (District): {layer_hits[1]/n:.2%} (Expect > 80%)")
        print(f"Layer 2 (Category): {layer_hits[2]/n:.2%} (The Goal)")
...
if __name__ == "__main__":
    ...
    # [关键] 指向刚刚训练好的 V4.1 模型
    GRPO = "/workspace/data/grpo_v4_1_breadcrumbs" 
